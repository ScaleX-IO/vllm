...
def test_decode_saving_consumer_delays_free_until_store_finishes():
    scheduler = _make_bare_scheduler(kv_role="kv_consumer", save_decode_cache=True)
    scheduler._request_trackers["req-0"] = RequestTracker(
        req_id="req-0",
        token_len=32,
        allocated_block_ids=([0, 1],),
        num_saved_tokens=32,
        prefill_end_tokens=16,
    )

    request = SimpleNamespace(request_id="req-0")

    assert scheduler.request_finished(request, ([0, 1],)) == (True, None)
