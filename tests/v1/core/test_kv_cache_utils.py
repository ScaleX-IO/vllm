...
def test_free_kv_cache_block_queue_score_order_uses_lru_for_ties():
    blocks = [KVCacheBlock(block_id=i) for i in range(5)]
    scores = [50, 10, 30, 10, 40]
    for block, score in zip(blocks, scores):
        block.eviction_score = score
    queue = FreeKVCacheBlockQueue(blocks)

    assert queue.popleft_n_by_score(2) == [blocks[1], blocks[3]]
    assert queue.get_all_free_blocks() == [blocks[0], blocks[2], blocks[4]]


def test_block_pool_score_rewards_reused_prefix_ancestors():
    pool = BlockPool(
        num_gpu_blocks=9,
        enable_caching=True,
        hash_block_size=16,
    ...
    scores = [block.eviction_score for block in blocks]
